-- 1. Create Ledger Status Enum
CREATE TYPE public.ledger_status AS ENUM ('pending', 'completed', 'failed');

-- 2. Alter Tables to support new requirements
ALTER TABLE public.orders
ADD COLUMN payment_method TEXT NOT NULL DEFAULT 'unknown';

ALTER TABLE public.ledger_entries
ADD COLUMN status public.ledger_status NOT NULL DEFAULT 'pending'::public.ledger_status;

-- 3. Drop the previous function since the return type changes from UUID to JSON
DROP FUNCTION IF EXISTS public.process_marketplace_checkout(jsonb);

-- 4. Create the updated RPC for marketplace checkout
